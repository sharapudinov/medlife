...
<div class="style-switcher<?=($_COOKIE['styleSwitcher'] == 'open' ? ' active' : '')?>">
									...
									<?foreach($arThematics as $arThematic):?>
										<div class="item col-md-4 col-sm-6 col-xs-12<?=($curThematic === $arThematic['CODE'] ? ' active' : '')?>" data-code="<?=$arThematic['CODE']?>">
											<div class="inner">
												<div class="img">
													<div class="img_inner">
														<img src="<?=$arThematic['PREVIEW_PICTURE']?>" alt="<?=$arThematic['TITLE']?>" title="<?=$arThematic['TITLE']?>" class="img-responsive">
													</div>
												</div>
											...
											<div class="preset-block<?=($arPreset['CURRENT'] ? ' current' : '')?><?=($arPreset['PREVIEW_PICTURE'] ? '' : ' no_img')?>" data-id="<?=$arPreset['ID']?>">
												<?if($arPreset['PREVIEW_PICTURE']):?>
													<div class="image">
														...
														<img src="<?=$arPreset['PREVIEW_PICTURE']?>" title="<?=$arPreset['TITLE']?>" class="img-responsive" />
													</div>
												<?endif;?>
												<div class="info">
													<div class="title"><?=$arPreset['TITLE']?></div>
													<div class="description" data-img="<?=$arPreset['DETAIL_PICTURE']?>"><?=$arPreset['DESCRIPTION']?></div>
												</div>
															...
															<div class="tabs bottom-line">
																<ul class="nav nav-tabs">
																	...
																	<?foreach(array_keys($arOption['OPTIONS']) as $key):?>
																		<?
																		$class = '';
																		if(isset($_COOKIE['styleSwitcherTabsCatalog']))
																		{
																			if($_COOKIE['styleSwitcherTabsCatalog'] == $j)
																				$class = 'active';
																		}
																		else
																		{
	...
	<div class="clearfix"></div>
	<?/*if(class_exists('\Aspro\Demo\Thematics')):?>
		<script>
		$(document).ready(function(){
			var cookieName = '<?=\Aspro\Demo\Thematics::getInstance()->thematicCookieName?>';
			var mainDomain = '<?=\Aspro\Demo\Thematics::getInstance()->mainDomain?>';
