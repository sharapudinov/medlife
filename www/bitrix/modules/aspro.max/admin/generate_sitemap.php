		...
		if($arTabsForView) {
			if(in_array($arSite['ID'], $arTabsForView)) {
				...
				$arTabs[] = array(
					...
					"OPTIONS" => array(
						"SITEMAP_URL" => array(
							"TITLE" => GetMessage("MAX_MODULE_SITEMAP_URL_TITLE"),
							"DEFAULT" => ($arSite["SERVER_NAME"] ? $arSite["SERVER_NAME"] : $_SERVER["SERVER_NAME"]),
							"TYPE" => "text",
							"REQUIRED" => "Y",
						),
						"SITEMAP_NAME" => array(
							"TITLE" => GetMessage("MAX_MODULE_SITEMAP_NAME_TITLE"),
							"DEFAULT" => "sitemap.xml",
							"TYPE" => "text",
							"REQUIRED" => "Y",
						),
						/*"SITEMAP_ADD_ROBOTS" => array(
							"TITLE" => GetMessage("MAX_MODULE_SITEMAP_ADD_ROBOTS_TITLE"),
							"TYPE" => "checkbox",
							"DEFAULT" => "N",
						),*/
					),
				...
				if(isset($arTab["OPTIONS"]) && $arTab["OPTIONS"])
				{
					foreach($arTab["OPTIONS"] as $optionCode => $arOption)
					{
						if(isset($arOption["REQUIRED"]) && $arOption["REQUIRED"] == "Y")
						{
							if(!$_POST[$optionCode."_".$optionsSiteID])
								$arErrors[$optionsSiteID][] = GetMessage('MAX_MODULE_FIELD_NO_VALUE', array('#FIELD#' => GetMessage('MAX_MODULE_'.$optionCode.'_TITLE'), "#SITE_ID#" => $optionsSiteID));
						}
						if($_POST[$optionCode."_".$optionsSiteID])
						{
							Option::set("aspro.max", $optionCode, $_POST[$optionCode."_".$optionsSiteID], $optionsSiteID);
						}
					}
					$siteMapName =  Option::get("aspro.max", "SITEMAP_NAME", $arTab["OPTIONS"]["SITEMAP_NAME"]["DEFAULT"], $optionsSiteID);
					$siteMapNewName =  Option::get("aspro.max", "SITEMAP_NEW_NAME", $arTab["OPTIONS"]["SITEMAP_NEW_NAME"]["DEFAULT"], $optionsSiteID);
					$siteMapUrl =  Option::get("aspro.max", "SITEMAP_URL", $arTab["OPTIONS"]["SITEMAP_URL"]["DEFAULT"], $optionsSiteID);
					$bExistSiteMap = (file_exists($arTab["SITE_DIR_FORMAT"].$siteMapName));
					if($arTab["HAS_REGIONS"])
					{
						if(!$bExistSiteMap)
							$arErrors[$optionsSiteID][] = GetMessage("MAX_MODULE_FILENAME_FULL", array("#FILE#" => $siteMapName, "#SITE_ID#" => $optionsSiteID));
					}
					...
					if(!$arErrors[$optionsSiteID])
					{
						if($arTab["ITEMS"])
						{
							...
							$LANDINGS_SEARCH_IBLOCK_ID = Cache::$arIBlocks[$optionsSiteID]['aspro_max_catalog']['aspro_max_search'][0];
...
\tRewriteRule ^(.*)$ /".$siteMapNameTmp.".php [L]", $file);
								...
								$arOK[$optionsSiteID][] = GetMessage("MAX_MODULE_FILENAME_GENERATE", array("#FILE#" => $siteMapName, "#SITE_ID#" => $optionsSiteID));
							}
						}
					}
				}
	...
	<?else:?>
		<?if($bShowGenerate):?>
			<div class="adm-info-message"><?=GetMessage("MAX_MODULE_SITEMAP_INFO");?></div>
			<br>
			<br>
		<?endif;?>
		<?$tabControl->Begin();?>
		...
		foreach($arTabs as $key => $arTab)
		{
			...
			if($arTab["SITE_ID"])
			{
				$optionsSiteID = $arTab["SITE_ID"];?>
				<?if(isset($arTab["OPTIONS"]) && $arTab["OPTIONS"]):?>
					<tr class="heading"><td colspan="2"><?=GetMessage("MAX_MODULE_MAIN_CONFIG")?></td></tr>
					<?foreach($arTab["OPTIONS"] as $optionCode => $arOption):?>
						<?$val = Option::get("aspro.max", $optionCode, $arOption["DEFAULT"], $optionsSiteID);?>
						<tr>
							<td>
								<?=$arOption["TITLE"];?>
							</td>
				...
				if(!$bExistSiteMap)
				{?>
					<td><?=GetMessage("MAX_MODULE_FILENAME", array("#FILE#" => $siteMapName))?></td>
					<td><?=GetMessage("MAX_MODULE_NOT_EXISTS")?></td>
				<?}?>
				<?
				if($arTab["HAS_REGIONS"])
				{
					if($arTab["ITEMS"])
					{
						foreach($arTab["ITEMS"] as $arItem):?>
							<tr>
							<td><?=GetMessage("MAX_MODULE_SITEMAP_DOMAIN", array("#DOMAIN#" => $arItem["PROPERTY_MAIN_DOMAIN_VALUE"]))?></td>
				...
				{?>
					<tr>
						<td style="width:100%;text-align:center;" colspan="2">
							<div class="adm-info-message"><?=GetMessage("MAX_MODULE_SITEMAP_ERROR");?></div>
						</td>
					</tr>
				<?}?>
			<?}
		...
		if($REQUEST_METHOD == "POST" && strlen($Generate.$Apply.$RestoreDefaults) && check_bitrix_sessid())
		{
			...
			<?$tabControl->Buttons();?>
			<?if($bShowGenerate):?>
				<input <?if($RIGHT < "W") echo "disabled"?> type="submit" name="Generate" class="submit-btn adm-btn-save" value="<?=GetMessage("MAX_MODULE_GENERATE_SITEMAP")?>" title="<?=GetMessage("MAX_MODULE_GENERATE_SITEMAP")?>">
			<?endif;?>
