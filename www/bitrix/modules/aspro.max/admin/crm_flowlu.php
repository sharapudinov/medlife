	...
	if($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults.$Auth.$SendCrm) && $RIGHT >= "W" && check_bitrix_sessid())
	{
		...
		if($Auth)
		{
			if($_POST["DOMAIN"] && $_POST["TOKEN"])
			{
				...
				echo \Aspro\Functions\CAsproMaxCRM::query($url, "/api/v1/module/crm/lead/list", $arPostFields);
			}
			else
			{
				$arError = array(
		...
		else
		{
			foreach($arTabs as $key => $arTab)
			{
				...
				if($domain && $token)
				{
					...
					$result = json_decode(\Aspro\Functions\CAsproMaxCRM::query($url, "/api/v1/module/crm/lead/list", $arPostFields), true);
