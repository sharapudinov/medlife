...
$(document).ready(function() {
	...
	var selectPreset = function(preset){
		...
		if($preset.length){
			...
			if(typeof arMaxOptions.PRESETS.LIST[preset] === 'object'){
				...
				var bPrepareWizard = (selectedThematic != arMaxOptions.THEMATICS.VALUE) && typeof arMaxOptions.THEMATICS.LIST[selectedThematic] === 'object' && !arMaxOptions.THEMATICS.LIST[selectedThematic].URL.length;
	...
	var setConfiguration = function(thematic, preset){
		if(typeof arMaxOptions.THEMATICS.LIST[thematic] === 'object'){
			if(typeof arMaxOptions.PRESETS.LIST[preset] === 'object'){
				if(thematic !== arMaxOptions.THEMATICS.VALUE){
	...
	$.ajax({
		...
		success:function(html){
			...
			$('.style-switcher .contents.updates .right-block .body_block').mCustomScrollbar({
				mouseWheel: {
					scrollAmount: 150,
					preventDefault: true
				}
	...
	$('.item.groups-tab a[data-toggle="tab"].linked').on('shown.bs.tab', function(e){
		...
		$.cookie('styleSwitcherTabsCatalog', _this.parent().index(), {path: '/'});
