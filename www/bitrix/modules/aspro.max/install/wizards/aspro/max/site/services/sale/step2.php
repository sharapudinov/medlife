...
if(COption::GetOptionString("aspro.max", "wizard_installed", "N", WIZARD_SITE_ID) != "Y")
{
	$locationGroupID = 0;
	$arLocationArr = Array();
...
COption::SetOptionString("aspro.max", "wizard_installed", "Y", false, WIZARD_SITE_ID);

if(\Bitrix\Main\Loader::includeModule('sale'))
{
	$arPerson = CSalePersonType::GetList(array('SORT' => 'ASC'), array('ACTIVE' => 'Y', 'LID' => WIZARD_SITE_ID), false, false, array())->Fetch();
	if($arPerson['ID'])
	{
		\Bitrix\Main\Config\Option::set('aspro.max', 'ONECLICKBUY_PERSON_TYPE', $arPerson['ID'], WIZARD_SITE_ID);
	}

	$arDeliveryServices = [];
	$dbRes = CSaleDelivery::GetList(array('SORT' => 'ASC'), array('ACTIVE' => 'Y', 'LID' => WIZARD_SITE_ID), false, false, array());
	while($arItem = $dbRes->Fetch())
	{
		$arDeliveryServices[$arItem['ID']] = $arItem['ID'];
		
	}
	if($arDeliveryServices)
	{
		$arDeliveryService = end($arDeliveryServices);
		\Bitrix\Main\Config\Option::set('aspro.max', 'ONECLICKBUY_DELIVERY', $arDeliveryService, WIZARD_SITE_ID);
	}

	$arPaymentService = [];
	$dbRes = CSalePaySystem::GetList(array('SORT' => 'ASC'), array('ACTIVE' => 'Y', 'LID' => WIZARD_SITE_ID), false, false, array());
	while($arItem = $dbRes->Fetch())
	{
		if($arItem['NAME'] == GetMessage("SALE_PAYMENT_CACSH"))
			$arPaymentService['ID'] = $arItem['ID'];
	}
	if($arPaymentService)
	{
		\Bitrix\Main\Config\Option::set('aspro.max', 'ONECLICKBUY_PAYMENT', $arPaymentService['ID'], WIZARD_SITE_ID);
	}
}
?>