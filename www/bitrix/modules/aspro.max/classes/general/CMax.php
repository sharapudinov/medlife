...
class CMax{
		...
		if($bStatic && $arValues === NULL || !$bStatic)
		{
			...
			if($arTemplate && $arTemplate['PATH'])
			{
				...
				if(isset(self::$arParametrsList['HEADER']['OPTIONS']['HEADER_TYPE']))
					self::Add2OptionCustomPageBlocks(self::$arParametrsList['HEADER']['OPTIONS']['HEADER_TYPE'], $arTemplate['PATH'].'/page_blocks/header/', 'header_custom', 'header_');

				// add custom values for FOOTER_TYPE
				if(isset(self::$arParametrsList['FOOTER']['OPTIONS']['FOOTER_TYPE']))
					self::Add2OptionCustomPageBlocks(self::$arParametrsList['FOOTER']['OPTIONS']['FOOTER_TYPE'], $arTemplate['PATH'].'/page_blocks/footer/', 'footer_custom', array('footer_'));

				// add custom values for HEADER_MOBILE
				if(isset(self::$arParametrsList['MOBILE']['OPTIONS']['HEADER_MOBILE']))
					self::Add2OptionCustomPageBlocks(self::$arParametrsList['MOBILE']['OPTIONS']['HEADER_MOBILE'], $arTemplate['PATH'].'/page_blocks/', 'header_mobile_custom');

				// add custom values for HEADER_MOBILE_MENU
				if(isset(self::$arParametrsList['MOBILE']['OPTIONS']['HEADER_MOBILE_MENU']))
					self::Add2OptionCustomPageBlocks(self::$arParametrsList['MOBILE']['OPTIONS']['HEADER_MOBILE_MENU'], $arTemplate['PATH'].'/page_blocks/', 'header_mobile_menu_custom');
		...
		if(CModule::IncludeModuleEx(self::moduleID) == 1)
		{
			...
			if(!$bIndexBot)
			{
				if(!$arTheme['CUSTOM_FONT'])
				{
					...
					$APPLICATION->AddHeadString('<link rel="preload" href="'.(CMain::IsHTTPS() ? 'https' : 'http').'://fonts.googleapis.com/css?family='.$font_family.'" as="style" crossorigin>');
				...
				if($arTheme['USE_LAZY_LOAD'] === 'Y'){
					$APPLICATION->AddHeadString('<script>window.lazySizesConfig = window.lazySizesConfig || {};lazySizesConfig.loadMode = 2;lazySizesConfig.expand = 300;lazySizesConfig.expFactor = 10;lazySizesConfig.hFac = 0.1;window.lazySizesConfig.lazyClass = "lazy";</script>');
    ...
    public static function Add2OptionCustomPageBlocks(&$arOption, $templateAbsPath, $filename, $replacer = array()){
	    			...
	    			if($arPageBlocks = glob($pageBlocksAbsPath.'/'.$filename.'*.php'))
	    			{
			    		foreach($arPageBlocks as $file)
			    		{
			    			...
			    			if($replacer) {
			    				...
			    				$replace = array_merge($replace, $replacer);
			    			}
			    			$file = str_replace($replace, '', basename($file));
			    			$arTmpFile = explode('_', $file);
							if(!isset($arOption['LIST'][end($arTmpFile)]))
							{
	...
	public static function getCurrentThemeClasses(){
		...
		return 'basket_'.$basket_class.' basket_fill_'.$arTheme['ORDER_BASKET_COLOR']['VALUE'].' side_'.$arTheme['SIDE_MENU']['VALUE'].' block_side_'.$arTheme['LEFT_BLOCK_CATALOG_SECTIONS']['DEPENDENT_PARAMS']['LEFT_BLOCK_CS_TYPE']['VALUE'].' catalog_icons_'.$arTheme['LEFT_BLOCK_CATALOG_ICONS']['VALUE'].' banner_auto '.($arTheme['USE_FAST_VIEW_PAGE_DETAIL']['VALUE'] != 'NO' ? 'with_fast_view' : '').' mheader-v'.$arTheme['HEADER_MOBILE']['VALUE'].' header-v'.$arTheme['HEADER_TYPE']['VALUE'].' header-font-lower_'.$arTheme['MENU_LOWERCASE']['VALUE'].' regions_'.$arTheme['USE_REGIONALITY']['VALUE'].' title_position_'.$arTheme['PAGE_TITLE_POSITION']['VALUE'].' fill_'.$arTheme['SHOW_BG_BLOCK']['VALUE'].' footer-v'.$arTheme['FOOTER_TYPE']['VALUE'].' front-v'.$arTheme['INDEX_TYPE']['VALUE'].' mfixed_'.$arTheme['HEADER_MOBILE_FIXED']['VALUE'].' mfixed_view_'.strtolower($arTheme['HEADER_MOBILE_FIXED']['DEPENDENT_PARAMS']['HEADER_MOBILE_SHOW']['VALUE']).' title-v'.$arTheme['PAGE_TITLE']['VALUE'].' lazy_'.$arTheme['USE_LAZY_LOAD']['VALUE'].((int)($arTheme['HEADER_PHONES']) > 0 ? ' with_phones' : '').($arTheme['MOBILE_CATALOG_LIST_ELEMENTS_COMPACT']['VALUE'] == 'Y' ? ' compact-catalog' : '').($arTheme['DARK_HOVER_OVERLAY']['VALUE'] == 'Y' ? ' dark-hover-overlay' : '');
	}
	...
	public static function showPageClass(){
		global $bLongBanner, $bLongHeader, $bLongHeader2, $bLongHeader3, $dopClass, $APPLICATION, $bLongBannerContents,$needLEftBlock;
	...
	public static function GetTotalCount($arItem, $arParams = array()){
		...
		if($arParams['USE_REGION'] == 'Y' && $arParams['STORES'])
		{
			...
			elseif(isset($arItem['SET_ITEMS']) && $arItem['SET_ITEMS'])
			{
				...
				if($totalCount > 0){
					...
					if($bDetail && $arItem["FRONT_CATALOG"] == "Y"){
						...
						elseif($arAddToBasketOptions["BUYMISSINGGOODS"] == "SUBSCRIBE" && $arItem["CATALOG_SUBSCRIBE"] == "Y"){
							$buttonText = array($arAddToBasketOptions['EXPRESSION_SUBSCRIBE_BUTTON'], $arAddToBasketOptions['EXPRESSION_SUBSCRIBED_BUTTON']);
							$buttonHTML = '<span class="'.$class_btn.' ss to-subscribe'.(!$bUserAuthorized ? ' auth' : '').(self::checkVersionModule('16.5.3', 'catalog') ? ' nsubsc' : '').' btn btn-default transition_bg" data-name="subscribe" data-param-form_id="subscribe" data-param-id="'.$arItem["ID"].'"  rel="nofollow" data-item="'.$arItem["ID"].'">'.self::showIconSvg("fw ncolor colored", SITE_TEMPLATE_PATH."/images/svg/subsribe_c.svg", $buttonText[0]).'<span>'.$buttonText[0].'</span></span><span class="'.$class_btn.' ss in-subscribe btn btn-default transition_bg" rel="nofollow" style="display:none;" data-item="'.$arItem["ID"].'">'.self::showIconSvg("fw ncolor colored", SITE_TEMPLATE_PATH."/images/svg/subsribe_c.svg", $buttonText[0]).'<span>'.$buttonText[1].'</span></span>';
						}
	...
	public static function updateExtendedReviewsProps($commentId, $action = '') {
		if(CModule::IncludeModule('blog') && CModule::IncludeModule('iblock')) {
			...
			if($comment) {
				$product = CIBlockElement::GetList(array(), array('PROPERTY_BLOG_POST_ID' => $comment['POST_ID']), false, array('nTopCount' => '1'), array('ID', 'PROPERTY_BLOG_POST_ID'))->Fetch();
				if($product) {
