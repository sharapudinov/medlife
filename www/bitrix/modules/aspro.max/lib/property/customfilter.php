...
class CustomFilter{
	static function OnIBlockPropertyBuildList(){
		return array(
			...
			'DESCRIPTION' => Loc::getMessage('MAX_CUSTOM_FILTER_PROP_TITLE'),
	...
	static function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName){
		...
		if($bDetailPage){
			if(!isset($cache)){
				$cache = array();
				$jsFile = '/bitrix/js/aspro.max/customfilter_control.js';
				$ajaxFile = '/bitrix/tools/aspro.max/customfilter_ajax.php';
								...
								'if(iv){'.
									...
									'if(td){'.
										...
										'if(tdd){'.
											...
											'}'.
										'}'.
										'initAsproMaxCustomFilterControl({'.
