...
if(!class_exists("CAsproMaxItem"))
{
	class CAsproMaxItem{
		...
		public static function showSectionGallery($arItem = array(), $key = 'GALLERY', $bReturn = false){
			if($arItem):?>
				<?ob_start();?>
					<?if($arItem[$key]):?>
							...
							<div class="wrapp_one_click">
								<?if(/*$arAddToBasketData["ACTION"] == "ADD" &&*/$arItem["OFFERS_MORE"] != "Y" && $arAddToBasketData["CAN_BUY"]):?>
										<?if($currentSKUID && $currentSKUIBlock):?>
											<span class="rounded3 colored_theme_hover_bg" data-item="<?=$currentSKUID?>" data-iblockID="<?=$currentSKUIBlock?>" data-quantity="<?=$arAddToBasketData["MIN_QUANTITY_BUY"];?>" onclick="oneClickBuy('<?=$currentSKUID?>', '<?=$currentSKUIBlock?>', this)" title="<?=Loc::getMessage('ONE_CLICK_BUY')?>">
										<?else:?>
											<span class="rounded3 colored_theme_hover_bg" data-item="<?=$arItem["ID"]?>" data-iblockID="<?=$arItem["IBLOCK_ID"]?>" data-quantity="<?=$arAddToBasketData["MIN_QUANTITY_BUY"];?>" onclick="oneClickBuy('<?=$arItem["ID"]?>', '<?=$arItem["IBLOCK_ID"]?>', this)" title="<?=Loc::getMessage('ONE_CLICK_BUY')?>">
										<?endif;?>
			...
			if((is_array($arParams) && $arParams)&& (is_array($arPrices) && $arPrices))
			{
				...
				foreach($arPrices as $key => $arPrice):?>
					<?if($arPrice["CAN_ACCESS"]):?>
						...
						<?if($iCountPriceGroup > 1):?>
							<div class="price_group <?=($arPrice['MIN_PRICE'] == 'Y' ? 'min ' : '');?><?=$arPriceGroup['XML_ID']?>"><div class="price_name muted font_xxs"><?=$arPriceGroup["NAME_LANG"]?></div>
						<?endif;?>
