...
if(!class_exists("CAsproMax"))
{
	class CAsproMax{
		...
		public static function showBlankImg($src = false){
			$bUseLazy = (Option::get(self::MODULE_ID, 'USE_LAZY_LOAD', 'Y', SITE_ID) == 'Y');
			...
			if(intVal($arParams["IBLOCK_".$field."_ID"]))
			{
				...
				if($arItems)
				{
					...
					foreach($arItems as $key => $arItem2)
					{
						...
						{
							$arTmpGoods = \json_decode($arItem2["~PROPERTY_LINK_GOODS_FILTER_VALUE"], true);
		...
		{?>
			<?ob_start();?>
				...
				<?if(\CMax::GetFrontParametrValue('SHOW_CALLBACK') != 'N'):?>
				...
				<?endif;?>
				<?if(\CMax::GetFrontParametrValue('SHOW_REVIEW') != 'N'):?>
				...
				<?endif;?>
				<?if(\CMax::GetFrontParametrValue('SHOW_QUESTION') != 'N'):?>
		...
		public static function showDiscountCounter($totalCount, $arDiscount = array(), $arQuantityData, $arResult, $strMeasure = '', $type = 'v2', $item_id = 0, $bWrapper = false){?>
			...
			if($arDiscount && $arDiscount["ACTIVE_TO"] && time() <= strtotime($arDiscount["ACTIVE_TO"])):?>
				...
				<?endif;?>
					<div class="view_sale_block <?=$type?> <?=($arQuantityData["HTML"] ? '' : 'wq');?>">
						<?if($type != 'compact'):?>
							<div class="icons">
								<div class="values">
					...
					<?if(!$bIframeMode):?>
						...
						<?if($bAll):?>
							<style type="text/css">
								...
								.ui-view-iframe-wrapper[data-type="mobile"] > div{padding: 56px 48px 27px 29px;}
								.ui-view-iframe-wrapper[data-type="mobile"] iframe{min-height: 782px;}
								.ui-view-iframe-wrapper[data-type="mobile"] > div > div{border-radius: 8px 8px 40px 40px;overflow: hidden;-webkit-backface-visibility: hidden;-moz-backface-visibility: hidden;-webkit-transform: translate3d(0, 0, 0); -moz-transform: translate3d(0, 0, 0);}
			...
			<?if($productId > 0 && $arParams['CALCULATE_DELIVERY'] !== 'NOT'):?>
				...
				<?ob_start();?>
					<div class="calculate-delivery text-form muted777 muted ncolor<?=($bWithPreview ? ' with_preview' : '')?>">
						...
						<span class="animate-load dotted font_sxs" data-event="jqm" data-param-form_id="delivery" data-name="delivery" data-param-product_id="<?=$productId?>"><?=$arParams['EXPRESSION_FOR_CALCULATE_DELIVERY']?></span>
				...
				if($arAllMatchValues)
				{
					...
					if($arPostFields)
					{
						foreach($arPostFields as $crm => $arFields)
						{
							...
							$result = \Aspro\Functions\CAsproMaxCRM::query($url, \Aspro\Functions\CAsproMaxCRM::$arCrmMethods[$crm]["CREATE_LEAD"], $arFieldsLead, $CURL, $DECODE);
							...
							if(isset($arCrmResult['response']))
							{
								if($crm == 'AMO_CRM' && $arCrmResult['response']['leads']) // create contact and company for amocrm
								{
									...
									if($arFields['notes_leads'])
									{
										...
										$resultNote = \Aspro\Functions\CAsproMaxCRM::query($url, \Aspro\Functions\CAsproMaxCRM::$arCrmMethods[$crm]["CREATE_NOTES"], $arFieldsNote, $CURL, $DECODE);
									...
									if($arCustomFields && $arCustomFields['companies'])
									{
										...
										$resultCompany = \Aspro\Functions\CAsproMaxCRM::query($url, \Aspro\Functions\CAsproMaxCRM::$arCrmMethods[$crm]["CREATE_COMPANY"], $arFieldsCompany, $CURL, $DECODE);
									...
									$resultContact = \Aspro\Functions\CAsproMaxCRM::query($url, \Aspro\Functions\CAsproMaxCRM::$arCrmMethods['AMO_CRM']['CREATE_CONTACT'], $arFieldsContact, $CURL, $DECODE);
