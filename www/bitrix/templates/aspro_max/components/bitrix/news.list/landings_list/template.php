		...
		<div class="landings-list__info">
			...
			<?foreach ($arResult['ITEMS'] as $key => $arItem) {
				...
				if ($url) {
					...
					if (
						(strpos($curDirDec, $urlDec) !== false) ||
						(strpos($curDir, $urlDec) !== false) ||
						(strpos($curDir, $urlDecCP) !== false)
					) {
						if($bActiveFilter){
							if ($bHidden) {
			...
			<div class="d-inline landings-list__info-wrapper <?=($bWithHidden ? 'last' : '');?>">
				<?foreach($arResult['ITEMS'] as $arItem):?>
					...
					<div class="landings-list__item font_xs" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
						<div>
							<?if(strlen($url)):?>
								<?if((strpos($curDirDec, $urlDec) !== false) || (strpos($curDir, $urlDec) !== false) || (strpos($curDir, $urlDecCP) !== false)):?>
