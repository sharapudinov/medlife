	...
	<script>
		...
		setBuyBlock = function(th, obj, index)
		{
			...
			setTimeout(function(){
				...
				$('.<?=$arPost["CLASS"]?>.js_offers__<?=$arPost["LINK_ID"]?>').closest('.catalog_block').find('.catalog_item_wrapp .item_info .sa_block').sliceHeight({resize: false, mobile: true});
				$('.<?=$arPost["CLASS"]?>.js_offers__<?=$arPost["LINK_ID"]?>').closest('.catalog_block').find('.catalog_item_wrapp .item_info .cost.prices').sliceHeight({resize: false, mobile: true});
